...
int main()
{
        ...
        Vehiculo ingresos[VEHICULOS_ARR_MAX_SIZE];
        Vehiculo egresos[VEHICULOS_ARR_MAX_SIZE];

        ingresar_entero("ingrese el tamano de la lista: ", &ingresos_size);
        for (int i = 0; i < ingresos_size; i++) {
                printf("Vehiculo [%d/%d]:\n", i, ingresos_size - 1);
                ingresar_vehiculo(&ingresos[i]);
        }
        ordenar_vehiculos(ingresos, ingresos_size);

        while (!salida) 
                menu(&salida, ingresos, egresos, &ingresos_size, &egresos_size);
...
void ingresar_vehiculo(Vehiculo *vehiculo)
{
        ...
        vehiculo->estancia.hora = 0;
        vehiculo->estancia.minutos = 0;
        vehiculo->importe = 0;
}
void egresar_vehiculo(Vehiculo *ingresos_p, Vehiculo *egresos_p, int *ingresos_size, int *egresos_size, int indice)
{
...
void mostrar_vehiculos(Vehiculo *vehiculos_p, int size)
{
        for (int i = 0; i < size; i++) {
                ...
                printf("estancia: %d:%d\n", vehiculos_p[i].estancia.hora, vehiculos_p[i].estancia.minutos);
